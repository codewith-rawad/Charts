class AreaChart extends Chart {
    ...
    updateVis(selectedCategory = 'Sports') {
        ...
        const areaData = Array.from(groupedData, ([year, values]) => ({
            year: year,
        ...
        vis.renderVis(selectedCategory, areaData);
    }

    renderVis(selectedCategory, areaData) {
        const vis = this;
        ...
        const x = d3.scaleBand().domain(areaData.map(d => d.year)).range([0, width]).padding(0.1);
        const y = d3.scaleLinear().domain([0, d3.max(areaData, d => d.sales)]).nice().range([height, 0]);
        ...
        legend.append('text')
            ...
            .attr('font-size', '12px')
            .attr('fill', 'yellow')
            .text(`Area for ${selectedCategory}`);
